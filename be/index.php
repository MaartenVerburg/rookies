<?php
	// ********************************** CMBEL *****************************************
	// Generates .ics (ical) calendar file from belgisn competitions on cmbel.shiftf5.be
	// exports 3 years, current, next and past year
	// URL Options: www.hexentric.nl/rookies/be/?events=[day]&action=[download]
	//
    ...
    $content1 = file_get_contents('https://cmbel.shiftf5.be/competitions/year/' . (date("Y") - 1));
		$first_step1 = explode( '<tbody>' , $content1 );
		$second_step1 = explode('</tbody>' , $first_step1[1] );

    $content2 = file_get_contents('https://cmbel.shiftf5.be/competitions/year/' . date("Y") );
    	$first_step2 = explode( '<tbody>' , $content2 );
		$second_step2 = explode('</tbody>' , $first_step2[1] );
		
	$content3 = file_get_contents('https://cmbel.shiftf5.be/competitions/year/' . date("Y") + 1 );
	...
	echo "X-WR-TIMEZONE:Europe/Brussels\r\n";
    ...
    foreach($rows as $n=>$item) {
        ...
        echo "BEGIN:VEVENT\r\n";
			...
			echo "SUMMARY:" . trim(addslashes($summary)) . "\r\n";
